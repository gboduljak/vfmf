...
class DINORawVAE(nn.Module):
  ...
  def encode(self, f: torch.Tensor, deterministic=False):
      B, T, H, W, _ = f.shape
      x = einops.rearrange(
          f,
          "b t h w c -> (b t) (h w) c"
      ...
      out = self.vae(x, deterministic=deterministic)
      z = einops.rearrange(out.latents, "(b t) h w c -> b t h w c", b=B, t=T)
      return z
  ...
  def decode(self, z: torch.Tensor):
      ...
      f = self.vae.decode(z)  # b, h, w, c
      if self.feature_per_channel_mean is not None:
          f = self.feature_inverse_transform(f)
      f = einops.rearrange(
          f, "(b t) h w c -> b t h w c",
          b=B
      )
      return f
